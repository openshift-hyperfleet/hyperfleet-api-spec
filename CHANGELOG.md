...
- GCP-specific Channel resource model with `ChannelSpec` (`is_default`, `enabled_regex`) and full CRUD service at `/channels` (HYPERFLEET-1083)
- GCP-specific Version resource model with `VersionSpec` (`raw_version`, `enabled`, `default`, `release_image`, `end_of_life_time`) and full CRUD service nested at `/channels/{channel_id}/versions` (HYPERFLEET-1083)
...
- `SearchParams` examples now use resource-agnostic queries (`name`, `labels`) instead of `status.conditions.Reconciled`
